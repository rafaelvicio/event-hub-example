...
class EventHubSender extends linkEntity_1.LinkEntity {
    ...
    constructor(context, partitionId, name) {
        ...
        this._onSessionError = (context) => {
            ...
            if (sessionError) {
                ...
                log.error("[%s] An error occurred on the session of sender '%s': %O.", this._context.connectionId, this.name, err);
            }
        };
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (senderError && !wasCloseInitiated) {
                    ...
                    log.error("[%s] Something is busted. State of sender '%s' with address '%s' is: %O", this._context.connectionId, this.name, this.address, state);
                }
                if (shouldReopen) {
                    yield amqp_common_1.defaultLock.acquire(this.senderLock, () => {
                        ...
                        const config = {
                            ...
                            operationType: amqp_common_1.RetryOperationType.senderLink,
                            times: amqp_common_1.Constants.defaultConnectionRetryAttempts,
                            delayInSeconds: 15
                        };
                        return amqp_common_1.retry(config);
                    });
                }
            }
            catch (err) {
    ...
    isOpen() {
        ...
        log.error("[%s] Sender '%s' with address '%s' is open? -> %s", this._context.connectionId, this.name, this.address, result);
        return result;
    }
    /**
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!this.isOpen()) {
                    ...
                    yield amqp_common_1.defaultLock.acquire(this.senderLock, () => { return this._init(); });
                }
    ...
    sendBatch(datas) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                log.sender("[%s] Sender '%s', sending encoded batch message.", this._context.connectionId, this.name, encodedBatchMessage);
    ...
    _deleteFromCache() {
        ...
        log.error("[%s] Deleted the sender '%s' with address '%s' from the client cache.", this._context.connectionId, this.name, this.address);
    }
    ...
    _trySend(message, tag, format) {
        const sendEventPromise = () => new Promise((resolve, reject) => {
            let waitTimer;
            log.sender("[%s] Sender '%s', credit: %d available: %d", this._context.connectionId, this.name, this._sender.credit, this._sender.session.outgoing.available());
            if (this._sender.sendable()) {
                ...
                const removeListeners = () => {
                    ...
                    this._sender.removeListener(rhea_promise_1.SenderEvents.rejected, onRejected);
                    this._sender.removeListener(rhea_promise_1.SenderEvents.accepted, onAccepted);
                    this._sender.removeListener(rhea_promise_1.SenderEvents.released, onReleased);
                    this._sender.removeListener(rhea_promise_1.SenderEvents.modified, onModified);
                };
                onAccepted = (context) => {
                ...
                onRejected = (context) => {
                    ...
                    const err = amqp_common_1.translate(context.delivery.remote_state.error);
                    log.error(err);
                    reject(err);
                };
                onReleased = (context) => {
                ...
                const actionAfterTimeout = () => {
                    ...
                    log.error(desc);
                    const e = {
                        condition: amqp_common_1.ErrorNameConditionMapper.ServiceUnavailableError,
                        description: desc
                    };
                    return reject(amqp_common_1.translate(e));
                };
                this._sender.on(rhea_promise_1.SenderEvents.accepted, onAccepted);
                this._sender.on(rhea_promise_1.SenderEvents.rejected, onRejected);
                this._sender.on(rhea_promise_1.SenderEvents.modified, onModified);
                this._sender.on(rhea_promise_1.SenderEvents.released, onReleased);
                waitTimer = setTimeout(actionAfterTimeout, amqp_common_1.Constants.defaultOperationTimeoutInSeconds * 1000);
                const delivery = this._sender.send(message, tag, format);
                log.sender("[%s] Sender '%s', sent message with delivery id: %d and tag: %s", this._context.connectionId, this.name, delivery.id, delivery.tag.toString());
            }
            else {
        ...
        const jitterInSeconds = amqp_common_1.randomNumberFromInterval(1, 4);
        const config = {
            operation: sendEventPromise,
            connectionId: this._context.connectionId,
            operationType: amqp_common_1.RetryOperationType.sendMessage,
            times: amqp_common_1.Constants.defaultRetryAttempts,
            delayInSeconds: amqp_common_1.Constants.defaultDelayBetweenOperationRetriesInSeconds + jitterInSeconds
        };
        return amqp_common_1.retry(config);
    }
    /**
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!this.isOpen() && !this.isConnecting) {
                    ...
                    log.error("[%s] Sender '%s' with address '%s' has established itself.", this._context.connectionId, this.name, this.address);
                    this._sender.setMaxListeners(1000);
                    log.error("[%s] Promise to create the sender resolved. Created sender with name: %s", this._context.connectionId, this.name);
